...
def test_vsearch(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/vsearch/config")
        data_path = Path(".tests/unit/vsearch/data")
        expected_path = Path(".tests/unit/vsearch/expected")
        ...
        check_output(
            [
                ...
                "stats/reads/derep/HD827sonic_2_lib2_collapsed.log",
                "--snakefile",
                "../../workflow/Snakefile",
                "-f",
                "--notemp",
