...
def test_loglog(conda_prefix):

    with tempfile.TemporaryDirectory() as tmpdir:
        workdir = Path(tmpdir) / "workdir"
        config_path = Path(".tests/unit/loglog/config")
        data_path = Path(".tests/unit/loglog/data")
        expected_path = Path(".tests/unit/loglog/expected")
        ...
        check_output(
            [
                ...
                "-m",
                "snakemake",
                "temp/reads/extend/loglog/HD827sonic_1_lib1_collapsed.flag",
                "--snakefile",
                "../../workflow/Snakefile",
                "-f",
                "--notemp",
